...
| A pathological directory (huge file count, an enormous single file) makes a request hang or exhaust memory. | `discover_tree` caps the number of entries collected (`MAX_TREE_ENTRIES`); `/api/file` and the Git-diff endpoint both refuse a file over `MAX_SERVABLE_FILE_SIZE` (issue #9). Neither cap is meant to be reached by an ordinary project -- they're backstops, not a feature. |
